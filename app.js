...
function renderOverall(q){
 ...
 arr.forEach((p)=>{
   ...
   boardEl.insertAdjacentHTML("beforeend",`<div class="player-row" data-player="${esc(p[0])}" data-points="${p[1]}" data-position="${p[2]}" data-region="${p[3]}"><div class="pos">${p[2]}.</div><div class="p-main"><span class="skin-frame"><img class="skin" src="${avatar(p[0])}" onerror="avatarFallback(this, '${esc(p[0])}')"></span><div><div class="p-name">${esc(p[0])}</div><div class="p-title">${p[2]===1?"Combat Grandmaster":p[2]<=3?"Combat Master":"Combat Ace"} (${p[1]} points)</div></div></div><div class="region ${p[3].toLowerCase()}">${p[3]}</div><div class="mini-tiers">${modesHtml}</div></div>`);
 });
}
...
function openPlayerModal(player, points, position, region){
  ...
  modalBody.innerHTML=`
    <div class="modal-player-top">
      ...
      <h2>${esc(player)}</h2>
      <div class="modal-rank">🏅 ${rankTitle}</div>
