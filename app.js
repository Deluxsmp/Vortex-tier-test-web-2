...
function renderOverall(q){
 ...
 arr.forEach(p=>{
  ...
  const cats=modeIds.map(id=>{const m=modeById(id),r=result(p,id),t=r?.tier||'';return `<div class="mini"><div class="mini-icon">${iconHTML(m,'mini-icon-img')||`<span class="icon-fallback">${m.i}</span>`}</div>${t?`<span class="mini-label ${tierClass(t)}">${esc(t)}</span>`:'<span class="mini-label empty-tier">—</span>'}</div>`}).join('');
...
function modalFor(p){
 ...
 const tiers=testedModes.map(id=>{const m=modeById(id),r=result(p,id);return `<div class="modal-tier"><div class="modal-tier-icon">${iconHTML(m,'mini-icon-img')}</div><b class="${tierClass(r?.tier||'')}">${r?.tier?esc(r.tier):'—'}</b>${r?.points!==null&&r?.points!==undefined&&r?.points!==''?`<small>${r.points} pts</small>`:''}</div>`}).join('');
