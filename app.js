...
function avatar(n){
  ...
  return `https://mc-api.io/render/BUST/${encodeURIComponent(name)}/JAVA?size=256`;
}
function avatarFallback(img,n){
  if(img.dataset.fallback!=='1'){
    img.dataset.fallback='1';
    img.src=`https://mc-api.io/render/BUST/${encodeURIComponent(n)}/JAVA?size=256`;
  }else{
...
const OVERALL_TIER_PATTERN=["HT1","HT1","HT1","HT1","HT1","HT1","LT1","LT1","HT2","LT2"];
function overallTierFor(player, modeId){
  const baseIndex=overall.findIndex(p=>p[0].toLowerCase()===player.toLowerCase());
  if(baseIndex<0) return "—";
  const modeIndex=Math.max(0,icons.indexOf(modeId));
  return OVERALL_TIER_PATTERN[(baseIndex+modeIndex)%OVERALL_TIER_PATTERN.length];
}
function renderOverall(q){
 ...
 arr.forEach((p)=>{
   const modesHtml=icons.map((id)=>{const mode=modeIconById[id];const t=overallTierFor(p[0],id);return `<div class="mini"><div class="mini-icon">${iconHTML(mode,"mini-icon-img")}${iconFallbackHTML(mode)}</div><span class="mini-label ${t.toLowerCase()}">${t}</span></div>`}).join("");
   const rankTitle = p[2]===1 ? "Combat Grandmaster" : p[2]<=3 ? "Combat Master" : "Combat Ace";
   const rankEmoji = p[2]===1 ? "👑" : p[2]<=3 ? "🏅" : "⚔️";
   const rankClass = p[2]===1 ? "rank-gold" : p[2]===2 ? "rank-silver" : p[2]===3 ? "rank-bronze" : "rank-normal";
   boardEl.insertAdjacentHTML("beforeend",`<div class="player-row ${rankClass}" data-player="${esc(p[0])}" data-points="${p[1]}" data-position="${p[2]}" data-region="${p[3]}"><div class="pos-wrap"><div class="pos">${p[2]}.</div></div><div class="p-main"><span class="skin-frame"><img class="skin" src="${avatar(p[0])}" onerror="avatarFallback(this, '${esc(p[0])}')"></span><div><div class="p-name">${esc(p[0])}</div><div class="p-title"><span class="rank-title-emoji">${rankEmoji}</span> ${rankTitle} <span class="points">(${p[1]} points)</span></div></div></div><div class="region ${p[3].toLowerCase()}">${p[3]}</div><div class="mini-tiers">${modesHtml}</div></div>`);
 });
}
...
function playerTier(player, modeId){
  // Overall rows and the player modal use the exact same tier source.
  // For players that are present in a mode board, show their real tier.
  const d=modeData[modeId];
  if(d){
    for(let t=1;t<=5;t++) if((d[t]||[]).some(n=>n.toLowerCase()===player.toLowerCase())) return `T${t}`;
  }
  // If the player is not listed in that mode board, mirror the tier shown
  // in the Overall row so the modal never disagrees with the leaderboard.
  return overallTierFor(player,modeId);
}
function openPlayerModal(player, points, position, region){
  const rankTitle=position===1?"Combat Grandmaster":position<=3?"Combat Master":"Combat Ace";
  const rankEmoji=position===1?"👑":position<=3?"🏅":"⚔️";
  const regionName={NA:"North America",EU:"Europe",ME:"Middle East",AS:"Asia"}[region]||region;
  const tierModes=["smp","pot","nethop","vanilla","sword","axe","mace","uhc"];
  const tiers=tierModes.map(id=>{
    const m=modeIconById[id]; const t=playerTier(player,id);
    return `<div class="modal-tier"><div class="modal-tier-icon">${iconHTML(m,"mini-icon-img")}${iconFallbackHTML(m)}</div><b class="${t.toLowerCase()}">${t}</b></div>`;
  }).join("");
  modalBody.innerHTML=`
    <div class="modal-player-top">
      <div class="modal-avatar"><img src="${avatar(player)}" alt="${esc(player)}" onerror="avatarFallback(this,'${esc(player)}')"></div>
      <h2>${esc(player)}</h2>
      <div class="modal-rank">${rankEmoji} ${rankTitle}</div>
      <div class="modal-region-text">${esc(regionName)}</div>
      <a class="namemc-link" href="https://namemc.com/profile/${encodeURIComponent(player)}" target="_blank" rel="noopener">◧ NameMC ↗</a>
    </div>
    <div class="modal-section-title">Position</div>
    <div class="modal-position-box"><span class="modal-position-number">${position}.</span><span>🏆 <strong>OVERALL</strong> <small>(${points} points)</small></span></div>
    <div class="modal-section-title">Tiers</div>
    <div class="modal-tiers-grid">${tiers}</div>`;
  modal.classList.remove("hidden");
}
boardEl.addEventListener("click",e=>{
  const row=e.target.closest(".player-row");
  if(!row) return;
  openPlayerModal(row.dataset.player, row.dataset.points, row.dataset.position, row.dataset.region);
});

document.getElementById("info").onclick=()=>modal.classList.remove("hidden");
