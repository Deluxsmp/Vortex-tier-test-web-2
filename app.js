...
function avatar(n){return "https://mc-heads.net/body/"+encodeURIComponent(n)+"/100.png"}
...
function renderOverall(q){
 ...
 arr.forEach((p,i)=>{
   const modesHtml=icons.map((ic,j)=>{const tiers=["HT1","HT1","HT1","HT1","HT1","HT1","LT1","LT1","HT2","LT2"];let t=tiers[(i+j)%tiers.length];return `<div class="mini"><div class="mini-icon">${ic}</div><span class="mini-label ${t.toLowerCase()}">${t}</span></div>`}).join("");
   boardEl.insertAdjacentHTML("beforeend",`<div class="player-row"><div class="pos">${p[2]}.</div><div class="p-main"><img class="skin" src="${avatar(p[0])}" onerror="this.style.visibility='hidden'"><div><div class="p-name">${esc(p[0])}</div><div class="p-title">${p[2]<=5?"Combat Grandmaster":"Combat Ace"} (${p[1]} points)</div></div></div><div class="region ${p[3].toLowerCase()}">${p[3]}</div><div class="mini-tiers">${modesHtml}</div></div>`);
 });
}
function renderMode(q){
 ...
 boardEl.innerHTML=`<div class="mode-board">${[1,2,3,4,5].map(i=>`<div class="tier-column tier-${i}"><div class="tier-head"><span>${i===1?"🏆":i===2?"🏆":"🏆"}</span>${tierNames[i]}</div><div class="tier-list">${filtered[i].length?filtered[i].map(n=>`<div class="tier-player"><img src="${avatar(n)}" onerror="this.style.visibility='hidden'"><span class="name">${esc(n)}</span><span class="up">⌃</span></div>`).join(""):`<div class="tier-player"><span class="name">No players</span></div>`}</div></div>`).join("")}</div>`;
}
