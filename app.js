...
function renderOverall(q){
 ...
 arr.forEach((p,i)=>{
   ...
   boardEl.insertAdjacentHTML("beforeend",`<div class="player-row"><div class="pos">${p[2]}.</div><div class="p-main"><img class="skin" src="${avatar(p[0])}" onerror="avatarFallback(this, '${esc(p[0])}')"><div><div class="p-name">${esc(p[0])}</div><div class="p-title">${p[2]<=5?"Combat Grandmaster":"Combat Ace"} (${p[1]} points)</div></div></div><div class="region ${p[3].toLowerCase()}">${p[3]}</div><div class="mini-tiers">${modesHtml}</div></div>`);
 });
}
function renderMode(q){
 ...
 boardEl.innerHTML=`<div class="mode-board">${[1,2,3,4,5].map(i=>`<div class="tier-column tier-${i}"><div class="tier-head"><span>${i===1?"🏆":i===2?"🏆":"🏆"}</span>${tierNames[i]}</div><div class="tier-list">${filtered[i].length?filtered[i].map(n=>`<div class="tier-player"><img src="${avatar(n)}" onerror="avatarFallback(this, '${esc(n)}')"><span class="name">${esc(n)}</span><span class="up">⌃</span></div>`).join(""):`<div class="tier-player"><span class="name">No players</span></div>`}</div></div>`).join("")}</div>`;
}
