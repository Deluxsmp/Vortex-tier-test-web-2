...
function secret(){return env('ADMIN_PASSWORD','change-this-password') + '|vortex-tier-session'}
...
function isAuthed(req){try{const v=cookies(req).vt_auth;if(!v)return false;const [payload,sig]=v.split('.');if(!payload||!sig)return false;const expected=sign(payload);if(sig.length!==expected.length)return false;if(!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))return false;const data=JSON.parse(Buffer.from(payload,'base64url').toString());return data.admin===true&&data.exp>Date.now()}catch{return false}}
...
async function github(method,apiPath,bodyObj){
 ...
 const repo='Deluxsmp/Vortex-tier-test-web-2'; const branch='main';
...
async function handler(req,res){res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Cache-Control','no-store');const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);const p=url.pathname.replace(/^\/api/,'')||'/';try{
 ...
 if(p==='/admin/login'&&req.method==='POST'){const b=await body(req);if(String(b.password||'')!==env('ADMIN_PASSWORD','@dhrubo2a'))return send(res,401,{error:'Invalid password'});const payload=Buffer.from(JSON.stringify({admin:true,exp:Date.now()+86400000})).toString('base64url');setCookie(res,'vt_auth',`${payload}.${sign(payload)}`,86400);return send(res,200,{ok:true})}
 if(p==='/admin/logout'&&req.method==='POST'){clearCookie(res,'vt_auth');return send(res,200,{ok:true})}
 if(p==='/admin/me'&&req.method==='GET'){if(!isAuthed(req))return send(res,401,{error:'Unauthorized'});return send(res,200,{ok:true})}
